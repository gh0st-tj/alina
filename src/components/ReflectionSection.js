            ...
            {points.map((point, index) => {
              ...
              return (
                <div 
                  key={index} 
                  ref={cardRef}
                  className="bg-white p-4 md:p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-500 opacity-0 transform hover:-translate-y-1"
                >
                  <div className="flex flex-col items-center text-center">
                    {icons[index]}
                    <h3 className="text-lg md:text-xl font-semibold mb-2 md:mb-3">{point.title}</h3>
